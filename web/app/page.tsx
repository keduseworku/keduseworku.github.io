...
export default function Home(){
  ...
  useEffect(()=>{
    const notes=Array.from(document.querySelectorAll<HTMLElement>('.personal-note'));
    const measure=()=>notes.forEach(note=>note.style.setProperty('--note-height',`${note.scrollHeight}px`));
    const observer=new ResizeObserver(measure);
    notes.forEach(note=>observer.observe(note));measure();
  ...
  return <>
    ...
    <header><a className="wordmark" href="#universe">KW<span className="brand-dot"/><span className="wordmark-note">KEDUSE WORKU</span></a><nav className="site-links" aria-label="Main navigation"><a href="/research/">Research</a><a href="/cv/">Resume</a><a href="#perspective">About</a><a href="mailto:keduseworku@aya.yale.edu">Email</a></nav></header>
    <main>
      <section id="universe" className="chapter opening"><div className="intro"><p className="eyebrow">01 / THE UNSEEN UNIVERSE</p><h1>Keduse<br/><em>Worku.</em></h1><p className="intro-copy">I work on astrophysics and inference.<br/>Here’s a little of what that looks like.</p><a className="enter" href="#field-orbit"><span className="arrow">↓</span> Take a look</a></div><span id="field-orbit" className="field-orbit-anchor"/><div className="research-note personal-note"><span className="research-rule"/><div>{progress<.17?<><p className="story-label">RELIC NEUTRINOS</p><h3 className="two-line-heading"><span>Neutrinos have been around</span><span>a lot longer than we have.</span></h3><p className="story-body">Some of my research is on relic neutrinos left over from the early universe. I study how gravity affects their motion.</p></>:progress<.28?<><p className="story-label">MODELING</p><h3>Where do the particles go?</h3><p className="story-body">In practice, my work has meant building probabilistic models and running large simulations. Here, I follow neutrino trajectories to calculate how they accumulate around dark-matter halos.</p></>:<><p className="story-label">HOW I THINK</p><h3>How much can we actually tell?</h3><p className="story-body">Most of my work involves pulling weak signals out of messy, high-dimensional systems. I use statistical modeling and Bayesian inference to work with datasets where uncertainty actually matters.</p></>}</div></div></section>
      <section id="lensing" className="chapter lens-chapter"><div className="lens-reading"><div className="chapter-copy lens-copy"><p className="eyebrow">02 / MACS0647 · GRAVITATIONAL LENSING</p><h2 className="two-line-heading"><span>A closer look at</span><em>distant galaxies.</em></h2><div className="lens-narrative"><p className={lensProgress<.3?'is-visible':''}>I’ve also worked with JWST observations. My collaborators and I studied distant galaxies behind a cluster called MACS0647, which magnifies their light.</p><p className={lensProgress>=.3&&lensProgress<.68?'is-visible':''}>The cluster’s gravity bends the light on its way to us. That helps us see faint galaxies, though it also stretches and distorts what we’re looking at.</p><p className={lensProgress>=.68?'is-visible':''}>We used images to find galaxy candidates, then spectroscopy to confirm redshifts. The distinction matters: a promising candidate still needs to be checked.</p></div><div className="research-stats"><div><strong>57</strong><span>galaxy candidates</span></div><div><strong>14</strong><span>confirmed redshifts</span></div><div><strong>9.25</strong><span>highest confirmed z</span></div></div><a className="text-link" href="/research/#macs-figure">Explore the MACS0647 research ↗</a><div className="lens-state" aria-hidden="true"><span>UNLENSED</span><div><i style={{transform:`scaleX(${lensProgress})`}}/></div><span>LENSED</span></div></div></div><div className="lens-exploration-note personal-note">{lensProgress<.65?<><p className="story-label">WORKING WITH DATA</p><h3>What’s in the light?</h3><p className="story-body">With imaging and spectroscopy, we can estimate stellar masses, star formation rates, and metallicities. That’s the part I work on: using the observations to learn about these early galaxies.</p></>:<><p className="story-label">COSMIC DAWN / 21-CM</p><h3>When did the first<br/>galaxies form?</h3><p className="story-body">Hydrogen gives us a way to study that period through its 21-cm signal. My work asks how magnetic fields left over from the early universe could change the timing.</p><p className="story-body story-followup">We extended Zeus21 to model how those fields affect early structure formation and the hydrogen signal. We then forecast what HERA and SKA could measure.</p><a className="text-link" href="/research/#cosmic-dawn">See the results and paper ↗</a></>}</div></section>
      <section id="perspective" className="chapter ending portrait-section"><div className="portrait-layout"><figure className="portrait"><img src="/media/keduse-worku.jpg" alt="Keduse Worku" width="1144" height="1092" loading="lazy"/></figure><div className="chapter-copy"><p className="eyebrow">03 / A LITTLE MORE ABOUT ME</p><h2>And beyond<br/><em>astrophysics.</em></h2><div className="about-copy"><p>I’m most excited by problems where prediction alone isn’t enough. I care about understanding how models behave, where they fail, and what tradeoffs matter.</p><p>I’ve applied these tools to studying the early universe, financial modeling, and AI evaluation. I also enjoy communicating these insights clearly, whether to fellow researchers, engineers, or anyone curious about the science.</p></div><div className="end-links"><a href="/research/">Explore my research ↗</a><a href="/cv/">Resume</a><a href="mailto:keduseworku@aya.yale.edu">Email me</a><a href="https://github.com/Keduseworku" target="_blank" rel="noreferrer">GitHub ↗</a></div></div></div><a className="return" href="#universe">↑ Return to the beginning</a></section>
    </main>
